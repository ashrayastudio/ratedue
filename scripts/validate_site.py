#!/usr/bin/env python3
"""Dependency-free structural and identity validation for the RateDue site."""
...
CANONICAL_ORIGIN = "https://ratedue.com"
PAGE_PATHS = {
    Path("index.html"): "/",
...
def run_self_test() -> int:
    ...
    for source in (
        ...
        if not any(
            ...
            for error in public_policy_errors(source, parser, allow_controller_disclosure=True)
        ):
            print("self-test accepted non-standard privacy controller copy", file=sys.stderr)
            return 1
    print("RateDue website validator self-test passed.")
    return 0
...
def main() -> int:
    ...
    for relative_path in expected_files:
        if not (ROOT / relative_path).is_file():
            errors.append(f"missing {relative_path}")
    if (ROOT / "CNAME").read_text(encoding="utf-8").strip() != "ratedue.com":
        errors.append("CNAME must contain only ratedue.com")
    ...
    if errors:
        print("RateDue website validation failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("RateDue website structural validation passed.")
    return 0
